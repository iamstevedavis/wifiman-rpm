...
grep -q 'type ifconfig_t;' "$BASE_TE"
grep -q 'type init_var_lib_t;' "$BASE_TE"
grep -q 'type iptables_t;' "$BASE_TE"
grep -q 'type node_t;' "$BASE_TE"
grep -q 'class netlink_generic_socket { bind create getattr read setopt write };' "$BASE_TE"
grep -q 'class file { append create getattr ioctl open read rename setattr unlink write };' "$BASE_TE"
grep -q 'class fifo_file open;' "$BASE_TE"
grep -q 'allow ifconfig_t init_var_lib_t:file { append ioctl };' "$BASE_TE"
grep -q 'allow iptables_t init_var_lib_t:file append;' "$BASE_TE"
grep -q 'allow iptables_t init_t:fifo_file open;' "$BASE_TE"
grep -q 'class rawip_socket { bind create getattr node_bind read setopt write };' "$BASE_TE"
grep -q 'allow init_t self:netlink_generic_socket { bind create getattr read setopt write };' "$BASE_TE"
grep -q 'allow init_t self:rawip_socket { bind create getattr read setopt write };' "$BASE_TE"
grep -q 'allow init_t node_t:rawip_socket node_bind;' "$BASE_TE"
